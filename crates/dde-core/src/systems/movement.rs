...
use crate::components::behavior::{MovementSpeed, PathfindingTarget};
...
impl MovementSystem {
    ...
    pub fn update(world: &mut World, tilemap: &TileCollisionMap, dt: f32) {
        ...
        for (entity, (pos, sub_pos, speed, target)) in world.query_mut::<(
            &Position,
            &mut SubPosition,
            &MovementSpeed,
            &PathfindingTarget,
        ...
        for (entity, new_x, new_y, new_px, new_py, direction) in updates {
            if let Ok((pos, sub_pos)) =
                world.query_one_mut::<(&mut Position, &mut SubPosition)>(entity)
            {
                pos.x = new_x;
                pos.y = new_y;
                sub_pos.px = new_px;
