...
use egui::{
    Color32, Id, Key, PointerButton, Pos2, Rect, RichText, Sense, Stroke,
    Ui, Vec2,
};

use super::debugger::{BtDebugger, draw_node_with_status, status_color};
...
impl BehaviorTreeEditor {
    ...
    fn draw_palette(&mut self, ui: &mut Ui) {
        ...
        egui::ScrollArea::vertical().show(ui, |ui| {
            for category in NodeCategory::all() {
                let header_text = format!("{} {}", category_icon(category), category.display_name());
                    ...
                    .show(ui, |ui| {
                        for template in category.templates() {
                            ...
                            if !self.palette_filter.is_empty()
                                && !name.to_lowercase().contains(&self.palette_filter.to_lowercase())
                            {
                                continue;
                            }
    ...
    fn draw_canvas(&mut self, ui: &mut Ui, debugger: Option<&BtDebugger>) {
        ...
        let canvas_response = ui.interact(canvas_rect, Id::new("bt_canvas"), Sense::click_and_drag());

        // Pan canvas
        if canvas_response.dragged_by(PointerButton::Middle) || 
           (canvas_response.dragged_by(PointerButton::Primary) && ui.input(|i| i.modifiers.shift)) {
    ...
    fn draw_node_recursive(&mut self, ui: &mut Ui, node: BtNode, debugger: Option<&BtDebugger>) {
            ...
            if response.drag_stopped() {
                ...
                if let Some(root) = &mut self.root {
                    if let Some(n) = root.find_node_mut(node_id) {
                        ...
                        // Snap to grid
                        if self.show_grid {
                            n.position[0] = (n.position[0] / self.grid_size).round() * self.grid_size;
                            n.position[1] = (n.position[1] / self.grid_size).round() * self.grid_size;
                        }
                    }
                }
            }
        ...
        draw_node_with_status(ui, node_rect, node_id, debugger, |ui| {
            ...
            ui.painter().rect_stroke(node_rect, 5.0, Stroke::new(2.0, color32));
            ...
            if let Some(comment) = &node.comment {
                ui.painter().text(
                    ...
                    if comment.len() > 15 { &comment[..15] } else { comment },
    ...
    fn draw_properties_panel(&mut self, ui: &mut Ui, debugger: Option<&BtDebugger>) {
        ...
        if let Some(selected_id) = self.selected {
            ...
            let node_clone = self.root.as_ref().and_then(|r| r.find_node(selected_id)).cloned();
    ...
    fn draw_node_properties(&mut self, ui: &mut Ui, node: &mut BtNode) {
        match &mut node.node_type {
            ...
            BtNodeType::Wait { seconds } => {
                ui.label("Seconds:");
                ui.add(egui::DragValue::new(seconds).speed(0.1).range(0.0..=f32::MAX));
            }
            BtNodeType::Cooldown { seconds, .. } => {
                ui.label("Cooldown (s):");
                ui.add(egui::DragValue::new(seconds).speed(0.1).range(0.01..=f32::MAX));
            }
            BtNodeType::Repeater { count, .. } => {
                ui.label("Repeat count (0 = forever):");
                let mut count_val = count.unwrap_or(0);
                if ui.add(egui::DragValue::new(&mut count_val).range(0..=1000)).changed() {
                    *count = if count_val == 0 { None } else { Some(count_val) };
                }
            }
            BtNodeType::Parallel { success_policy, failure_policy, .. } => {
    ...
    fn duplicate_node(&mut self, node_id: NodeId) {
        let node_to_clone = self.root.as_ref().and_then(|r| r.find_node(node_id)).cloned();
...
fn draw_parallel_policy_combo(ui: &mut Ui, policy: &mut super::nodes::ParallelPolicy) {
    egui::ComboBox::from_id_source("parallel_policy")
        ...
        .show_ui(ui, |ui| {
            ui.selectable_value(policy, super::nodes::ParallelPolicy::RequireAll, "Require All");
            ui.selectable_value(policy, super::nodes::ParallelPolicy::RequireOne, "Require One");
        });
}
