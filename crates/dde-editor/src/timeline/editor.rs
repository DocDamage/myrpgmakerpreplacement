...
pub enum Selection {
    ...
    Multiple { keyframes: Vec<(TrackId, usize)> },
    /// Track selected
    Track(TrackId),
    /// Time range selected
    TimeRange {
...
impl TimelineEditor {
    ...
    fn draw_toolbar(&mut self, ui: &mut egui::Ui) {
        ui.horizontal(|ui| {
            ...
            // Loop toggle
            let loop_text = if self.loop_playback {
                "🔁 On"
            } else {
                "🔁 Off"
            };
            ...
            // Snap toggle
            let snap_text = if self.snap_enabled {
                "🧲 On"
            } else {
                "🧲 Off"
            };
            ...
            if ui
                .button("Fit")
                .on_hover_text("Fit timeline to view")
                .clicked()
            {
                self.fit_to_view();
            }
        ...
        while t <= end_time {
            ...
            for i in 1..5 {
                ...
                if minor_x >= rect.min.x && minor_x <= rect.max.x {
                    painter.line_segment(
                        [
                            egui::pos2(minor_x, rect.max.y - 8.0),
        ...
        for (i, track) in self.tracks.iter().enumerate() {
            ...
            if let Some(_value) = track.value_at(self.playhead) {
                ...
                if line_x >= rect.min.x && line_x <= rect.max.x {
                    painter.line_segment(
                        [
                            egui::pos2(line_x, lane_y + 5.0),
                            egui::pos2(line_x, lane_y + lane_height - 5.0),
                        ],
                        Stroke::new(
                            1.0,
