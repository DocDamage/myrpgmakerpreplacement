...
use crate::visual_script::{
    compile_to_events,
    compiler::{graph_from_json, graph_to_json},
    CompiledScript, NodeCanvas, NodeGraph, ScriptExecutor,
};
...
impl VisualScriptEditor {
    ...
    fn draw_toolbar(&mut self, ui: &mut egui::Ui) {
        ui.horizontal(|ui| {
            ...
            if ui
                .button("🔨 Compile")
                .on_hover_text("Compile script (F5)")
                .clicked()
            {
                self.compile();
            }
            if ui
                .button("▶️ Test")
                .on_hover_text("Test run script (F6)")
                .clicked()
            {
                self.test_run();
            }
    ...
    fn draw_node_palette(&mut self, ui: &mut egui::Ui) {
        ...
        // Search filter
        ui.add(
            egui::TextEdit::singleline(&mut self.palette_filter).hint_text("🔍 Search nodes..."),
        );
        ui.add_space(8.0);
    ...
    fn draw_properties_panel(&mut self, ui: &mut egui::Ui) {
        ui.horizontal(|ui| {
            ui.selectable_value(
                &mut self.property_panel.active_tab,
    ...
    fn draw_node_properties(&mut self, ui: &mut egui::Ui) {
        if let Some(node) = self.canvas.first_selected_node() {
            ...
            ui.label(format!(
                "Connections: {}",
                self.canvas.graph().connections.len()
            ));
            ui.label(format!(
                "Event Nodes: {}",
                self.canvas.graph().get_event_nodes().len()
            ));
        }
    }
    ...
    fn draw_compile_info(&mut self, ui: &mut egui::Ui) {
        ...
        if let Some(ref result) = self.last_compile_result {
            match result {
                Ok(script) => {
                    ui.label(
                        egui::RichText::new("✓ Compilation Successful").color(egui::Color32::GREEN),
                    );
                    ui.add_space(8.0);
